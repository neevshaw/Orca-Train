...
def adaptive_coarsegrain_gpu(ar, countar, cutoff=5, max_levels=8, min_shape=8):
    ...
    #     countar = torch.from_numpy(countar)
        ...
        if not np.allclose(Nlog, np.rint(Nlog)):
            newN = np.int_(2 ** np.ceil(Nlog))  # next power-of-two sized matrix
...
def _adaptive_coarsegrain(ar, countar, max_levels=12, cuda=True):
    """
...
class Genomic2DFeatures(Target):
    ...
    def __init__(self, input_paths, res, features, shape, cg=False, cuda=False):
        """
    ...
    def get_feature_data(self, chrom, start, end, chrom2=None, start2=None, end2=None):
        if not self._initialized:
            self.data = [hicstraw.HiCFile(path) for path in self.input_paths]
            self._initialized = True
        self.chrom = chrom
        self.start = start
        self.end = end
        if chrom2 is not None and start2 is not None and end2 is not None:
            c_query = (chrom.replace('chr', '', 1) if 'chr' in chrom else chrom, chrom2.replace('chr', '', 1) if 'chr' in chrom2 else chrom2)
            bp_query = (start,  end, start2, end2)
        else:
            c_query = (chrom.replace('chr', '', 1) if 'chr' in chrom else chrom, chrom.replace('chr', '', 1) if 'chr' in chrom else chrom)
            bp_query = (start,  end, start, end)
        if self.cg:
            out = []
            for c, res in zip(self.data, self.res):
                s1 = bp_query[0]
                e1 = bp_query[1]-res
                s2 = bp_query[2]
                e2 = bp_query[3]-res
                ...
                zd = c.getMatrixZoomData(*c_query,"observed","KR","BP",res)
                recs = zd.getRecords(s1,  e1, s2, e2)  # sparse
                
                B = np.zeros((N1, N2), dtype=np.float32)

                # pseudo-code: depends on record fields
                for r in recs:
                    i = (r.binX - s1)//1000
                    j = (r.binY - s2)//1000
                    if 0 <= j < N1 and 0 <= i < N2:
                        B[i, j] = r.counts
                        B[j, i] = r.counts


                zd = c.getMatrixZoomData(*c_query,"observed","NONE","BP",res)
                recs = zd.getRecords(s1,  e1, s2, e2)  # sparse
                ...
                for r in recs:
                    i = (r.binX - s1)//1000
                    j = (r.binY - s2)//1000
                    if 0 <= i < N2 and 0 <= j < N1:
                        R[i, j] = r.counts
                        R[j, i] = r.counts

                A = _adaptive_coarsegrain(B, R).astype(np.float32).reshape(250,4,250,4).mean(axis=(-1,-3))
                out.append(A)
        else:
            out = [
                c.getMatrixZoomData(*c_query, "observed", "KR", "BP", r).getRecordsAsMatrix(bp_query[0], bp_query[1]-r, bp_query[2], bp_query[3]-r).astype(np.float32).reshape(-1,4,250,4).mean(axis=(-1,-3))
                for c,r in zip(self.data, self.res)
            ]
        if len(out) == 1:
            out = out[0]
        else:
