...
if __name__ == "__main__":
    ...
    normmat_bydist = np.exp(
        np.load("../resources/4DNFI2TK7L2F.rebinned.hic.expected.res1000.npy")
    )[:1000]
    normmat = normmat_bydist[np.abs(np.arange(1000)[:, None] - np.arange(1000)[None, :])]

    t = Genomic2DFeatures(
        ["../resources/4DNFI2TK7L2F.hic"],
        [1000],
        ["r1000"],
        (1000, 1000),
        cg=True,
    ...
    dataloader = SamplerDataLoader(sampler, num_workers=21, batch_size=16)
